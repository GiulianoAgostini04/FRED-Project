...
The project is complemented by a VHDL module that digitally models the channel-selection logic and the classification finite-state machine, mainly for educational purposes (not necessarily synthesized on physical FPGA).
...
- **PCB (FRED)** — custom EEG acquisition board: full analog front-end (electrodes → protection → channel multiplexer → instrumentation amplifier → filtering → galvanic isolation → output buffer) and digital channel-selection block.
- **PC layer** — OpenBCI Ganglion + BrainFlow extract alpha/beta band power via FFT and send the data serially to Arduino.
- **Arduino layer** — receives the processed data, drives the 16x2 LCD display (EEG frequency, band, "Focused"/"Not focused" state) and LEDs based on the alpha/beta ratio. USB-powered.
...
- **Open DRC issue:** a "Rectangle" graphic object on Sig_Top accidentally assigned to the GND net, overlapping Edge.Cuts → clearance violation to be fixed manually
- Mounting holes missing — to be evaluated
...
Electrodes → diode clamp protection → CD4052B multiplexer → INA128 instrumentation amplifier → Sallen-Key high-pass filter (fc = 1 Hz) → amplification stage (A = 10) → 50 Hz Twin-T notch filter → galvanic isolation (NE555 PWM modulation + 4N35 optocoupler + demodulation) → anti-aliasing band-pass filter → output buffer → Arduino ADC.

DRL (Driven Right Leg) stage in parallel with the electrode connectors for common-mode rejection.
...
3. For VHDL simulation: `ghdl -a` → `ghdl -e` → `ghdl -r --wave=output.ghw` → `gtkwave output.ghw`
